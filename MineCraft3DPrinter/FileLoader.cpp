...
/*******************************************************************
									INTERFACE
*********************************************************************/
BOOL IFileLoader::ImportFile_STL(std::string pFilePath, std::vector<VECTOR3>& refVertexBuffer, std::vector<UINT>& refIndexBuffer, std::vector<VECTOR3>& refNormalBuffer, std::string & refFileInfo)
{
	std::ifstream tmpFile(pFilePath, std::ios::binary);

	if (!tmpFile.good())
	{
		LOG_MSG("Load STL : file Open Failed!!");
		return FALSE;
	}
	...
	{
		LOG_MSG("Load STL : file Damaged!!File Size is Too Small!!");
		return FALSE;
	}
	tmpFile.seekg(0);
...
/*******************************************************************
										PRIVATE
*********************************************************************/
BOOL IFileLoader::mFunction_ImportFile_STL_Binary(std::string pFilePath, std::vector<VECTOR3>& refVertexBuffer,
	std::vector<UINT>& refIndexBuffer, std::vector<VECTOR3>& refNormalBuffer, std::string& refFileInfo)
{
	...
	if (!fileIn.good())
	{
		LOG_MSG("Load STL Binary : Open File Failed!");
		return FALSE;
	}
	...
	if (headerInfo[0] == 's')
	{
		LOG_MSG("Load STL Binary : File Damaged!! It's not binary STL file!");
		return FALSE;
	}
...
#define REINTERPRET_READ(var) \
	...
	{
		LOG_MSG("Load STL Binary : Triangle Count is larger than 500000 / Data Damamged!!");
		fileIn.close();
		return FALSE;
	}
...
BOOL IFileLoader::mFunction_ImportFile_STL_Ascii(std::string pFilePath, std::vector<VECTOR3>& refVertexBuffer, std::vector<UINT>& refIndexBuffer, std::vector<VECTOR3>& refNormalBuffer, std::string& refFileInfo)
{
	std::ifstream fileIn(pFilePath);

	if (!fileIn.good())
	{
		LOG_MSG("Load STL Ascii : Open File Failed!!");
		return FALSE;
	}
	...
	{
		LOG_MSG("Load STL Ascii : file damaged!!");
		return FALSE;
	}
